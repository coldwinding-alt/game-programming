...
| 2026-05-26 | AI-regenerated tournament portrait atlas refresh | Partial pass | Rebuilt `Resources/mlp/Portraits/portraits_ui.png` from an AI-redrawn `8`-portrait source sheet, repacked it onto a `2048x1024` power-of-two atlas, and pinned the importer to `nPOTScale: None` so the JSON crop coordinates remain stable; offline bbox comparison against the previous template stayed aligned, while a final in-editor visual pass is still needed in the open Unity session |
